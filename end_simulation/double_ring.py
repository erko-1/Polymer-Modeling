...
angular_shift = theta / 2 if num_monomers % 2 == 1 else 0
...
# Closest monomers between rings
...
bonded_pairs += [(i, num_monomers + ((i - num_monomers + 1) % num_monomers)) for i in range(num_monomers, num_monomers_total)]
...
def total_energy(polymer):
    ...
    for i in range(num_monomers_total):
        for j in range(i+1, num_monomers_total):
            if j != i:
...
def get_internal_vectors(polymer, num_monomers, offset=0):
    vectors = []
    for i in range(num_monomers // 2):
        j = (i + num_monomers // 2) % num_monomers
        vec = minimum_image_distance(polymer[offset + j], polymer[offset + i], box_size)
...
def compute_gyration_tensor_total(polymer):
...
for frame in range(num_steps):
    ...
    angle_between_coms.append(np.arccos(dot_product))
    ...
    if frame != 0 and frame % 100 == 0:
        ...
        gyr_total = compute_gyration_tensor_total(polymer)
        Rg_squared_ring1.append(np.sum(np.linalg.eigvalsh(gyr1)))
        EV_ring1.append(np.linalg.eigvalsh(gyr1))
        Rg_squared_ring2.append(np.sum(np.linalg.eigvalsh(gyr2)))
        EV_ring2.append(np.linalg.eigvalsh(gyr2))    
        Rg_squared_total.append(np.sum(np.linalg.eigvalsh(gyr_total)))
        EV_total.append(np.linalg.eigvalsh(gyr_total))

    print(frame)


with open('2_DR_angles5.txt', 'w') as file:
    for step in range(num_steps):
        file.write(f'{step} {angle_between_coms[step]}\n')

with open('2_DR_Rg_squared_values5.txt', 'w') as file:
    file.write("Step_Range\tRg_squared_ring1\tRg_squared_ring2\tRg_squared_total\n")
    for i in range(len(Rg_squared_ring1)):
        start = i * 100
        end = (i + 1) * 100
        file.write(f'{start}-{end}\t{Rg_squared_ring1[i]}\t{Rg_squared_ring2[i]}\t{Rg_squared_total[i]}\n')

with open('2_DR_Eigenvalues5.txt', 'w') as f:
    f.write("Step\tEV_Ring1_1\tEV_Ring1_2\tEV_Ring1_3\t"
            "EV_Ring2_1\tEV_Ring2_2\tEV_Ring2_3\t"
            "EV_Total_1\tEV_Total_2\tEV_Total_3\n")
    for i in range(len(EV_total)):
        EV_ring1_str = '\t'.join(map(str, EV_ring1[i]))
        EV_ring2_str = '\t'.join(map(str, EV_ring2[i]))
        EV_total_str = '\t'.join(map(str, EV_total[i]))
        f.write(f'{i * 100}-{(i + 1) * 100}\t{EV_ring1_str}\t{EV_ring2_str}\t{EV_total_str}\n')
