...
Rg_squared_ring1 = []
Rg_squared_ring2 = []
initial_internal_vectors = get_internal_vectors(polymer, num_monomers, offset=0)
...
for frame in range(num_steps):
    ...
    current_vectors = get_internal_vectors(polymer, num_monomers, offset=0)
    correlation = np.mean([np.dot(current_vectors[i], initial_internal_vectors[i]) for i in range(len(current_vectors))])
