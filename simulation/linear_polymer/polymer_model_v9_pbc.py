...
# Function to initialize polymer with optimal bond length and PBC
def initialize_polymer(num_monomers, bond_length, box_size):
    polymer = np.zeros((num_monomers, 3))
    direction = np.array([1, 0, 0])  # Start along x-axis

    for i in range(1, num_monomers):
        new_position = polymer[i - 1] + bond_length * direction  # Step in the chosen direction
        new_position = apply_pbc(new_position, box_size)  # Apply PBC

        polymer[i] = new_position

        # Randomly change direction (small angles to prevent excessive bending)
        if i % 3 == 0:  # Change direction occasionally
            theta = np.random.uniform(0, np.pi / 3)  # Small deviation
            ...
            direction = new_direction / np.linalg.norm(new_direction)  # Normalize

    return polymer

# Initialize polymer
polymer = initialize_polymer(num_monomers, optimal_bond_length, box_size)
