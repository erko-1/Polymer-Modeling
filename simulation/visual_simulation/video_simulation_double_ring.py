...
ring2 = np.array([(np.cos(angles-angular_shift) * radius) + shift_distance,
                  np.sin(angles-angular_shift) * radius,
                  np.zeros(num_monomers)]).T

polymer = np.vstack((ring1, ring2))
num_monomers_total = 2 * num_monomers

# Adjust bridge connection
...
bonded_pairs += [(i, num_monomers + ((i - num_monomers + 1) % num_monomers))
...
def update(frame):
    ...
    for i in range(num_monomers - 1):
        ax.plot([polymer[i, 0], polymer[i + 1, 0]], [polymer[i, 1], polymer[i + 1, 1]], [polymer[i, 2], polymer[i + 1, 2]], color='b')
    ...
    # Gyration tensor (every 500 steps)
...
writer = Writer(fps=20, metadata=dict(artist='Simulation'), bitrate=1800)
ani.save("polymer_simulation.mp4", writer=writer)
