...
T = 1.0
k_B = 1.0
num_steps = 200  # Reduced for visualization
step_size = 0.1
optimal_bond_length = 0.9651375794516794

# Initialize polymer positions (figure-eight configuration)
a = 3.0  # Scaling factor for the shape
t_values = np.linspace(-np.pi/2, np.pi/2, num_monomers)
polymer = np.array([
    a * np.cos(t_values),               # X-coordinates
    a * np.sin(t_values) * np.cos(t_values),  # Y-coordinates
