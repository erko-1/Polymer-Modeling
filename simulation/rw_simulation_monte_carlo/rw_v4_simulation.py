...
def animate_simulation(trajectory, box_size, save_as_video=False):
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    points, = ax.plot([], [], 'o', color='red', alpha=0.7, markersize=20)
    ...
    if save_as_video:
        anim.save("random_walk_simulation_v4.gif", writer=PillowWriter(fps=10))  # Save as GIF
    else:
        plt.show()
...
temperature = 0.1      # Simulation temperature of the system (for monte carlo / boltzmann validation). Real Temperature is given by T = T_coded * epsilon/boltzmann_factor.
