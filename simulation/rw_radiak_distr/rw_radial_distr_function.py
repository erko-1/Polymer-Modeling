...
# Function to calculate the truncated Lennard-Jones potential
...
def total_potential_energy_optimized(positions, box_size, epsilon=1.0, sigma=1.0, cell_size=None):
    ...
    for idx, position in enumerate(positions):
        ...
        cell_index_1d = cell_idx[0] * num_cells + cell_idx[1] #2D grid is represented as a 1D array for simplicity and efficiency in memory storage
    ...
    for c_idx, cell in enumerate(cells):
        ...
        neighbors = []
        x, y = divmod(c_idx, num_cells) #1D Array into 2D
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                nx, ny = (x + dx) % num_cells, (y + dy) % num_cells #wrapping behavior
                neighbors.append(nx * num_cells + ny) #2D stored as 1D
...
temperature = 0.5        # Simulation temperature
...
final_positions = trajectory[-1]    # Use the last step of the trajectory for RDF calculation
